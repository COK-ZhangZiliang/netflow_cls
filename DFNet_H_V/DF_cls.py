...
timestamp = time.time()
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(message)s', filename=f"../results/H_V_{timestamp}.log", )
...
def load_and_transform_data(data_path):
    ...
    data_for_cls = data_for_cls.apply(lambda x: x[:500] if len(x) >= 500 else x + [0] * (500 - len(x)))
