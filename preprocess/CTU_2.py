...
if __name__ == '__main__':
    for i in range(13):
        os.makedirs(f'../datasets/CTU-13-Dataset/proc/{i+1}', exist_ok=True)
        bng_file = f'../datasets/CTU-13-Dataset/proc/{i+1}/bng.csv'
        mal_file = f'../datasets/CTU-13-Dataset/proc/{i+1}/mal.csv'
    ...
    for i in range(13):
        ...
        # Save data
        bng_data.to_csv(f'../datasets/CTU-13-Dataset/proc/{i+1}/bng.csv', mode='a', header=False, index=False)
        mal_data.to_csv(f'../datasets/CTU-13-Dataset/proc/{i+1}/mal.csv', mode='a', header=False, index=False)
